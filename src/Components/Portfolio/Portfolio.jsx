...
function Portfolio() {
  ...
  return (
    <>
      ...
      <div className="slider-container mt-24 px-4 cursor-pointer">
        <Slider {...settings}>
          {portfolio.map((item) => {
            return (
              <div key={item.id} className='px-5 relative hover:scale-110 transition delay-75 duration-300 ease-in-out'>
                    <div className='relative' >
                      {item.Path}
                      <div  className='cursor-pointer hover:bg-gradient-to-r from-[#f81f01] to-[#ee076e] h-full absolute opacity-75 z-10 w-full top-0 left-0'></div>
                    </div>
